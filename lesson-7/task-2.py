...
# пальто и костюм. У этих типов одежды существуют параметры: размер (для пальто) и рост (для костюма). Это м
# огут быть обычные числа: V и H, соответственно.
...
suit = Suit(175)
print(f'{suit}, рост {suit.height}, расход ткани: {suit.cloth_rate}')

coat = Coat(52)
print(f'{coat}, размер {coat.size}, расход ткани: {coat.cloth_rate}')
